...
void linear_vertical_axis (void) {
	#asm
		.lva_jmp
			...
			cp  PLAYER_JUMPING_FRAMES
			jr  nc, lva_jmp_shutdown
		...
		.lva_jmp_do
			...
			// (PLAYER_JUMPING_FRAMES/2) < p_cont_salto
			// p_cont_salto is in B
			ld  hl, (_p_jump_vy)
			ld  a, PLAYER_JUMPING_FRAMES/2
			cp  b
			jr  c, lva_jmp_set

			ld  de, 0
			ex  de, hl
			sbc hl, de 			// Carry is off
		...
		.lva_jmp_off
			ld  hl, PLAYER_LINEAR_GRAVITY<<FIXBITS
			ld  (_p_vy), hl
